...
- Ember eyebrow. Two chip rows: country chips ("All" first, then every country present, most-populated first) above genre chips ("All" first, then every genre present sorted by label in the active language). The country chips show the country name only (no flag). Both filters combine (AND) and each survives navigating away and back. Station cards: gradient tile, name, genre · kbps, red LIVE dot. Click streams the station through the engine, so the EQ and the live visualiser apply exactly as they do to a local file. The card shows "Connecting…" while the stream buffers, then "Live". Active station gets an ember border. Search filters by station name **and** genre label, within the active country and genre.
- The catalog ships ~980 stations across 21 countries (Europe, the UK and the USA) and ~45 genres, baked in as static JSON. It merges the hand-picked flagship streams — the SomaFM channels, the Radio France webradios (FIP, France Musique, Mouv'), Radio Paradise mixes, Radio Swiss, French independents (TSF Jazz, Nova, Meuh, Jazz Radio, FG, OÜI FM, Latina), US public/independent stations (KEXP, KCRW, WFMU, KUSC, WQXR, The Jazz Groove, SmoothJazz.com), international independents (NTS, Rinse FM, dublab, Venice Classic) and electronic specialists (Nightwave Plaza, Nightride FM, Ambient Sleeping Pill, Hirschmilch, Sunshine Live, Bassdrive) — with the most-listened stations per country from the [Radio Browser](https://www.radio-browser.info) community database, deduplicated and genre-mapped at build time. No network call is made to browse; only the chosen stream is opened.
