...
18. **Domain logic lives behind the right boundary.** Metadata parsing, library scanning, persistence and filesystem access live in **Rust** behind IPC commands that return pure data (wire types in `src/lib/models/`). Audio decoding and the EQ/analyser DSP live in the **Web Audio API** (`features/player/audioGraph.ts`) — that is a deliberate exception to "no complexity frontend-side": the webview's media stack is the decoder, and PCM never crosses IPC. The frontend renders data; it does not re-derive what Rust already computed.
...
| `mcp__context7__resolve-library-id` + `mcp__context7__get-library-docs` | Tauri v2 plugin APIs, Web Audio API — use live docs, not training memory |
...
- **Audio:** HTML5 `<audio>` + Web Audio API (10 peaking BiquadFilters + AnalyserNode) in the webview; `lofty` (Rust) for tags/properties/cover art; files served through Tauri's asset protocol with a runtime-scoped allowlist
...
The static scope in `tauri.conf.json` is **empty**. Every watched folder / imported file is allowed at runtime (`app.asset_protocol_scope()`), and `library::restore_asset_scope` re-grants the library's paths at every launch — nothing outside the library is reachable from the webview.

### Playback data flow

```
LibraryScreen/…  → playerStore.playQueue(tracks, i)
                    → audioGraph.element.src = convertFileSrc(path)
                    → element → 10×BiquadFilter → Analyser → destination
RadioScreen      → playerStore.playStation(station)
                    → plain second <audio> (no graph: CORS-less streams would be silenced)
eqStore          → audioGraph.applyGains(gains)          (allowed edge eq → player)
WaveformCanvas / SpectrumPanel → visualizer.tick(...)    (real analyser data, or synthetic fallback)
```
...
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- |
| **Rune class with setter methods** in `*Store.svelte.ts` | Persistent or side-effecting state (IPC, audio elements, Web Audio graph). Private `$state` + getters. | `playerStore`, `eqStore`, `libraryStore`, `preferencesStore`, `languageStore` |
